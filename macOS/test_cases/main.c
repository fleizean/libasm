...
extern size_t ft_strlen(const char *dest);
...
void strlenTester(char *str1, char *str2)
{
    ...
    if (len == len2) {
        ...
        printf(BBLU "(INF_TEST):: ft_strlen - %ld %s\n", len2, reset);
    }
    else {
        printf(BRED "strlen -> KO\n" reset);
        printf(BRED "(INF_TEST):: strlen - %ld %s\n", len, reset);
        printf(BRED "(INF_TEST):: ft_strlen - %ld %s\n", len2, reset);
    }
}
...
{
    if (strcmp(strdup(str1), ft_strdup(str2)) == 0) {
...
void writeTester(char *str1, char *file_path)
{
    ...
    if (fd != -1) {
        bool result = ((size_t)write(fd, str1, strlen(str1)) == strlen(str1)) && ((size_t)ft_write(fd, str1, strlen(str1)) == strlen(str1));
        if (result == true)
    ...
    ssize_t val6 = ft_write(wrong_fd, str1, 40);
    
    if (val5 == val6 && errno == 0) {
        printf(BGRN "write (wrong_fd test)        -> OK\n" reset);
...
void readTester(char *file_path) {
    ...
    if (val1 == val2 && errno == 0) {
        ...
        printf(BBLU "(INF_TEST):: ft_read - %ld\n" reset, val2);
    }
    ...
    ssize_t val6 = ft_read(wrong_fd, buf6, 40);
    if (val5 == val6 && errno == 0) {
        printf(BGRN "read (wrong_fd test)   -> OK\n" reset);
    } else {
        printf(BRED "read (wrong_fd test)   -> KO: %s\n" reset, strerror(errno));
        printf(BBLU "(INF_TEST):: read - %ld %s\n", val5, reset);
        printf(BBLU "(INF_TEST):: ft_read - %ld %s\n", val6, reset);
    }
}
